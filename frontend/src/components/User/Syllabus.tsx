...
const SyllabusTreeItem: React.FC<{
  ...
  return (
    <div>
      <button
        className={`flex w-full items-center justify-between rounded-xl px-3 py-2 text-left transition hover:bg-white/5 ${selected === node.id ? 'bg-primary/20 text-primary shadow' : ''
          }`}
      ...
      <motion.div initial={false} animate={{ height: open ? 'auto' : 0, opacity: open ? 1 : 0 }} transition={{ duration: 0.2 }}>
        {open && node.children.length > 0 && (
          <div className="ml-4 border-l border-white/10 pl-3">
            {node.children.map((child: SyllabusNode) => (
...
export const Syllabus: React.FC = () => {
  ...
  const loadSyllabus = useCallback(async () => {
    ...
    try {
      ...
      if (res.tree.length > 0) setSelectedSection((prev) => prev || res.tree[0].id);
    } catch (err: any) {
